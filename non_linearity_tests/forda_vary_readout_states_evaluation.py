#%%
"""
Evaluate FordA with varying number of readout states using parameters from a single Optuna study.

This script:
1. Loads the best parameters from a single Optuna study
2. Varies only the number of states taken from mid_states (readout window)
3. Evaluates performance across multiple seeds for each readout states value
4. Generates a line plot showing test accuracy vs number of readout states
"""
...
SOURCE_STUDY_NAME = "only_last_state_as_readout_20_units_not_aligned_less_ang_magnitude_no_tanh"

# Number of units (fixed)
N_UNITS = 20

# Number of readout states to evaluate (how many timesteps from mid_states to use)
# None means use all states (full sequence length)
N_READOUT_STATES_LIST = [4,8,12,16,20]  # None = all states
...
def create_input_maps(params, n_units, seed, ang_input=True):
    ...
    num_non_zero = min(num_non_zero, n_units)
...
#%%
def slice_mid_states(mid_states, n_units, n_readout_states):
    """
    Slice mid_states to get the first n_readout_states (x, y) pairs.
    
    mid_states has shape [batch, 2*n_units] where:
    - mid_states[:, 0:n_units] = x coordinates
    - mid_states[:, n_units:2*n_units] = y coordinates
    
    Returns mid_states with shape [batch, 2*n_readout_states] containing
    the first n_readout_states x's and corresponding y's.
    """
    if n_readout_states is None:
        return mid_states  # Use all states
    
    # Take first n_readout_states x-coordinates and first n_readout_states y-coordinates
    return torch.cat([
        mid_states[:, :n_readout_states],                    # first n_readout x's
        mid_states[:, n_units:n_units + n_readout_states]    # first n_readout y's
    ], dim=1)

#%%
@torch.no_grad()
def test_esn(data_loader, model, classifier, scaler, device, n_units, n_readout_states):
    """Test ESN performance with specified number of readout states"""
    activations, ys = [], []
    for x, labels in data_loader:
        ...
        # Slice to get first n_readout_states (x, y) pairs
        mid_states = slice_mid_states(mid_states, n_units, n_readout_states)
...
#%%
def single_evaluation(params, n_units, n_readout_states, seed, device, 
                      train_loader, valid_loader, test_loader,
                      ang_input=True, ang_connections=True, aligned_orientations=False):
    """Run a single evaluation with given seed and n_readout_states"""
    ...
    # Evaluate
    valid_score = test_esn(valid_loader, model, classifier, scaler, device, n_units, n_readout_states)
    test_score = test_esn(test_loader, model, classifier, scaler, device, n_units, n_readout_states)
    
    return valid_score, test_score, n_classifier_params

#%%
def evaluate_readout_states(params, n_units, n_readout_states, device, 
                            train_loader, valid_loader, test_loader, seeds,
                            ang_input=True, ang_connections=True, aligned_orientations=False):
    """Evaluate a single n_readout_states value across multiple seeds."""
    readout_str = "all" if n_readout_states is None else str(n_readout_states)
    print(f"\n  Evaluating n_readout_states={readout_str}")
    ...
    for seed in seeds:
        try:
            valid_score, test_score, n_classifier_params = single_evaluation(
                params, n_units, n_readout_states, seed, device, 
                train_loader, valid_loader, test_loader,
    ...
    if len(valid_scores) == 0:
        print(f"  No successful evaluations for n_readout_states={readout_str}")
        return None
    
    # Calculate statistics
    results = {
        'n_readout_states': n_readout_states if n_readout_states is not None else -1,  # -1 indicates "all"
        'n_readout_states_label': readout_str,
    ...
    print(f"  Results: Valid={results['valid_mean']:.4f}±{results['valid_std']:.4f}, "
          ...
          f"Classifier params={classifier_params}")
    
    return results

#%%
def main():
    """Main function to evaluate varying n_readout_states with fixed parameters."""
    print("="*70)
    print("VARYING READOUT STATES EVALUATION FOR FordA")
    print("="*70)
    print(f"Source study: {DATABASE_NAME}/{SOURCE_STUDY_NAME}")
    print(f"n_units: {N_UNITS}")
    print(f"n_readout_states values: {N_READOUT_STATES_LIST}")
    ...
    # Evaluate each n_readout_states value
    print("\n" + "="*70)
    print("EVALUATING DIFFERENT N_READOUT_STATES VALUES")
    print("="*70)
    
    all_results = []
    
    for n_readout_states in N_READOUT_STATES_LIST:
        result = evaluate_readout_states(
            params, N_UNITS, n_readout_states, device, 
            train_loader, valid_loader, test_loader,
    ...
    # Sort by n_readout_states (with "all" at the end)
    df = df.sort_values('n_readout_states')
    ...
    # Print table
    print("\n" + df[['n_readout_states_label', 'valid_mean', 'valid_std', 
                    'test_mean', 'test_std', 'classifier_params']].to_string(index=False))
    
    # Save to CSV
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    csv_filename = f"{parent_dir}/results/forda_vary_readout_states_{timestamp}.csv"
    os.makedirs(os.path.dirname(csv_filename), exist_ok=True)
    df.to_csv(csv_filename, index=False)
    print(f"\nResults saved to: {csv_filename}")
    
    # Create line plot of test accuracy vs n_readout_states
    plt.figure(figsize=(10, 6))
    
    # For plotting, replace -1 with a large value for "all" to put it at the end
    plot_df = df.copy()
    max_readout = plot_df[plot_df['n_readout_states'] > 0]['n_readout_states'].max()
    plot_df.loc[plot_df['n_readout_states'] == -1, 'n_readout_states'] = max_readout * 1.5
    
    plt.errorbar(plot_df['n_readout_states'], plot_df['test_mean'], 
                 yerr=plot_df['test_std'], 
                 marker='o', markersize=8, capsize=5, capthick=2, 
                 linewidth=2, elinewidth=1.5, linestyle='--', color='b', label='Test Accuracy')
    
    plt.xlabel('Number of Readout States', fontsize=12)
    plt.ylabel('Mean Test Accuracy', fontsize=12)
    plt.grid(True, alpha=0.3)
    
    # Set x-axis ticks with labels
    tick_positions = plot_df['n_readout_states'].values
    tick_labels = plot_df['n_readout_states_label'].values
    plt.xticks(tick_positions, tick_labels)
    
    plt.tight_layout()
    
    # Save plot
    plot_filename = f"{parent_dir}/plots/forda_vary_readout_states_{timestamp}.png"
...
if __name__ == "__main__":
    ...
    if results_df is not None:
        print("\n" + "="*70)
        print("FINAL RESULTS (sorted by n_readout_states)")
        print("="*70)
        for _, row in results_df.iterrows():
            print(f"n_readout_states={row['n_readout_states_label']:>4s}: "
