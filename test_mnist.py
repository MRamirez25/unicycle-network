...
non_zero_values_min = 10.0
non_zero_values_max = 15.0
...
model = UnicycleReservoir(n_inp=1, n_units=n_units, dt=0.01, n_out=3, lin_input_map=lin_input_map, 
...
def test(data_loader):
    ...
    with torch.no_grad():
        for i, (images, labels) in enumerate(data_loader):
            ...
            _, output = model(images, angular_input)
...
for epoch in range(n_epochs):
    ...
    for images, labels in progress_bar:
        ...
        optimizer.zero_grad()
        # with torch.no_grad():
        states_list, output = model(images, angular_input)
...
model.ang_input_map = model.ang_input_map.to(device)
model.unicycle_network.lin_damping = model.unicycle_network.lin_damping.to(device)
model.unicycle_network.ang_damping = model.unicycle_network.ang_damping.to(device)*10
model.unicycle_network.mass_vector = model.unicycle_network.mass_vector.to(device)
model.unicycle_network.j_vector = model.unicycle_network.j_vector.to(device)*0.01
