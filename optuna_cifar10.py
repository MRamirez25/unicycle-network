...
from utils import get_FordA_data, get_mnist_data, get_cifar_data
import time
...
def objective(trial, aligned_orientations=None, ang_input=None, ang_connections=None):
    ...
    ang_damping_min = trial.suggest_float('ang_damping_min', 0.01, 5.0)
    ang_damping_max = trial.suggest_float('ang_damping_max', ang_damping_min, 10.0)
    bs = trial.suggest_int("batch_size", 100, 300, step=50)
    ...
    # Fraction of total elements to make non-zero
    ...
    random_values = torch.rand(num_non_zero) * (magnitude_max - non_zero_values) + non_zero_values
    ...
    if ang_input:
        ...
        non_zero_values_ang = trial.suggest_float("magnitude_min_ang", -10, 0.0, step=2)
        magnitude_max_ang = trial.suggest_float("magnitude_max_ang", non_zero_values_ang, 10, step=2)
        random_values_ang = torch.rand(num_non_zero_ang) * (magnitude_max_ang - non_zero_values_ang) + non_zero_values_ang
    ...
    n_steps_readout = 0#trial.suggest_int("steps_readout", 0, 50, step=10)
    ...
    model = UnicycleReservoir(
        n_inp=n_inp, n_units=n_units, dt=dt, n_out=len(classes),
    ...
    rand_train = rands.repeat(bs,1,1)
    rand_test = rands.repeat(bs,1,1)
    ...
    validation_accuracy = test_esn(valid_loader, model, classifier, scaler, bs_test,rand_test)
    test_accuracy = test_esn(test_loader, model, classifier, scaler, bs_test,rand_test)
...
@torch.no_grad()
def test_esn(data_loader, model, classifier, scaler, bs_test, rand_test):
    activations, ys = [], []
...
if __name__ == '__main__':
    ...
    # study_name = "only_last_state_as_readout_1000_units"
    ...
    # study.optimize(partial(objective, aligned_orientations=True, ang_input=None, ang_connections=None), timeout=3600*4, n_trials=7)
    ...
    # database_name = "unicycle_nets_cifar10_logreg"
    # study_name = "only_last_state_as_readout_200_units"
    # storage_name = "sqlite:///optuna_databases/{}.db".format(database_name)
    # study = optuna.create_study(storage=storage_name, study_name=study_name, direction='maximize', load_if_exists="True")
    # # for trial in study.trials:
    # #     if trial.datetime_complete and trial.datetime_start:
    ...
    #         duration = trial.datetime_complete - trial.datetime_start
    #         print(f"Trial {trial.number} took {duration.total_seconds()} seconds")
    study.optimize(partial(objective, aligned_orientations=False, ang_input=True, ang_connections=True), timeout=3600*8)
