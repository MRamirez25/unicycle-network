...
def objective(trial, aligned_orientations=None, ang_input=None, ang_connections=None, n_units=20):
    ...
    bs = trial.suggest_int("batch_size", 100, 500, step=50)
    ...
    if ang_input:
        num_non_zero_ang = trial.suggest_int("non_zero_elements_ang", int(0.1*n_units), n_units)
        non_zero_indices = torch.randperm(n_units)[:num_non_zero_ang]  # Randomly select indices
        non_zero_values_ang = trial.suggest_float("magnitude_min_ang", -10, 0.0, step=5)
        magnitude_max_ang = trial.suggest_float("magnitude_max_ang", non_zero_values_ang, 10, step=1)
    ...
    n_connections_fraction = trial.suggest_float("n_connections_fraction", 0.1, 1.0, step=0.1)    
    ...
    n_steps_readout = None#trial.suggest_int("steps_readout", 0, 100, step=10)
    ...
    classes = [0,1]  # Binary classification for FordA
    ...
    n_past_steps_readout=0).to(device)
...
if __name__ == '__main__':
    ...
    # study_name = "only_last_state_as_readout_10_units_aligned_omega_fixed"
    ...
    # study.optimize(partial(objective, aligned_orientations=True, ang_input=None, ang_connections=None), timeout=3600*4)
