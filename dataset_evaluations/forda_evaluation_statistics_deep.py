...
N_UNITS_PER_LAYER = [20, 15]  # Units for each layer, e.g., [20, 15] for 2-layer network
# If N_LAYERS=2 and this is single int, will expand to [N_UNITS_PER_LAYER]*N_LAYERS
...
# For FordA with final layer units: mid_states has shape (batch, n_units_final*5)
# Common choices: (0, n_units_final*2) for x,y positions only, (0, n_units_final*5) or None for all features
FEATURE_SLICE_START = 0
FEATURE_SLICE_STOP = N_UNITS_PER_LAYER[-1]*2  # Use None for all features from final layer

linear_transform_scale = 2.0  # Scale for position selectors (deprecated, kept for compatibility)
...
def initialize_model(params, lin_input_map, ang_input_map, device):
    ...
    # Extract parameters for first layer
    n_connections_fraction = params['n_connections_fraction']
    n_connections = int(n_units_per_layer[0] * n_connections_fraction)
    anchor_con_fraction = params['anchor_con_fraction']
    n_connections_anchor = int(n_units_per_layer[0] * anchor_con_fraction)
    
    # Angular connections
    if ANG_CONNECTIONS:
        n_connections_ang_fraction = params['n_connections_ang_fraction']
        anchor_con_fraction_ang = params['anchor_con_fraction_ang']
        n_connections_anchor_ang = int(anchor_con_fraction_ang * n_units_per_layer[0])
        n_connections_ang = int(n_connections_ang_fraction * n_units_per_layer[0])
    else:
    ...
    model = DeepUnicycleReservoir(
        ...
        linear_transform_scale=linear_transform_scale
    ).to(device)
...
def single_evaluation(params, seed, device, train_loader, valid_loader, test_loader):
    ...
    lin_input_map, ang_input_map = create_input_maps(params, N_UNITS_PER_LAYER[0] if isinstance(N_UNITS_PER_LAYER, list) else N_UNITS_PER_LAYER, seed)
    ...
    model.set_init_states(params['batch_size'], x_per_layer, z_per_layer, theta_per_layer, s_per_layer, omega_per_layer)
...
def main():
    ...
    if len(valid_scores) > 0:
        ...
        print("\n" + "="*50)
        print("EVALUATION STATISTICS (DEEP NETWORK)")
        print("="*50)
...
• Classifier params: {classifier_params}
        ...
        plot_filename = f"{parent_dir}/plots/forda_evaluation_stats_deep_{STUDY_NAME}.png"
...
if 'results' in locals() and results is not None:
    ...
    results_filename = f"{parent_dir}/results/forda_stats_results_deep_{timestamp}.txt"
    os.makedirs(os.path.dirname(results_filename), exist_ok=True)
    with open(results_filename, 'w') as f:
        f.write(f"FordA Deep Evaluation Statistics - {STUDY_NAME}\n")
